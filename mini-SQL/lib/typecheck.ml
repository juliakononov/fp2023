...
let float_of_bool x = if x then 1. else 0.
  ...
  let (#+) (x: item) (y: item) = match x with
  (* INT + ... *)
  | Numeric x -> (match y with 
  ...
  let (#-) (a: item) (b: item) = match a with
  (* INT - ... *)
  | Numeric x -> (match b with 
    | Numeric y -> return (Numeric (x - y))
    | Real y -> return (Real ((float_of_int x) -. y))
  ...
  | Numeric x -> (match b with 
    ...
    | Real y -> is_zero_float y >>= fun y -> return (Real (mod_float (float_of_int x) y))
    | String _ -> (fail (TypesMismatch (a, "%" ,b)))
    | Bool _ -> (fail (TypesMismatch (a, "%" ,b))))
  (* FLOAT * ... *)
  | Real x -> (match b with 
    | Numeric y -> is_zero_int y >>= fun y -> return (Real (mod_float x (float_of_int y)))
